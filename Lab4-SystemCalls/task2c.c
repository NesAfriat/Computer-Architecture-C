...
int main(int argc, char *argv[], char *envp[])
{
  ...
  while (tmp < count)
  {
    ...
    else if (withPref == 2 && ((entp)->buf[0] == prefix))
    {
      system_call(SYS_WRITE, output, (entp)->buf, strlen(entp->buf));
      system_call(SYS_WRITE, output, "\t", 1);
      infection();
  ...
  /* print adress */
  system_call(SYS_WRITE, output, "code start: ", 13);
  system_call(SYS_WRITE, output, itoa((unsigned int)&code_start), 50);
  system_call(SYS_WRITE, output, " code end: ", 14);
  system_call(SYS_WRITE, output, itoa((unsigned int)&code_end), 50);
  system_call(SYS_WRITE, output, "\n", 1);
