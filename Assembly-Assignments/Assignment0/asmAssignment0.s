...
do_Str:                        		; do_Str function definition - functions are defined as labels
	...
	yourCode:			; use label to build a loop for treating the input string characters
                cmp byte [ecx],32       ; compare the value in ecx to ' '
                je space                       
                cmp byte [ecx], 90      ; check if the value is less then 'Z'
                jg end
                cmp byte [ecx], 65      ; check if the value is geater then 'A'
                jl end
                add byte [ecx], 32      ; change the letter to lowercase
                jmp end
        space:
                inc dword[an]           ; increment the space counter
        end:
