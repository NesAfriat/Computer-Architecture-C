...
void PrintSection()
{
    Elf32_Shdr *strTable = &elfSectionsHeader[elfHeader->e_shstrndx];
    const char *const nameOff = map_start + strTable->sh_offset;
    if (debug_mode == 1)
    {
    ...
    for (int i = 0; i < elfHeader->e_shnum; i++)
    {
        ...
        printf("[%d]\t", i);
        printf("%-20s\t", nameOff + elfSectionsHeader[i].sh_name);
