...
void PrintSection()
{
    Elf32_Shdr *sections = &elfSectionsHeader[elfHeader->e_shstrndx];
    const char *const section_name = map_start + sections->sh_offset;
    if (debug_mode == 1)
    {
    ...
    for (int i = 0; i < elfHeader->e_shnum; i++)
    {
        ...
        printf("[%d]\t", i);
        printf("%-20s\t", section_name + elfSectionsHeader[i].sh_name);
...
void printNameAndValue(int symIndex)
{
    Elf32_Shdr *sections = (Elf32_Shdr *)(map_start + elfHeader->e_shoff);
    for (int i = 0; i < elfHeader->e_shnum; i++)
    {
        if (sections[i].sh_type == SHT_DYNSYM)
        {
            ...
            char *symbol_names = (char *)(map_start + sections[sections[i].sh_link].sh_offset);
            if (debug_mode == 1)
                fprintf(stderr, "the reloc symbol's index is: %d\n", symIndex);
            printf("%08x\t", elfSymbolTable[symIndex].st_value);
            printf("%s\n", symbol_names + elfSymbolTable[symIndex].st_name);
        }
    }
}
...
void RelocationTables()
{
    ...
    for (int i = 0; i < elfHeader->e_shnum; i++)
    {
        ...
        {
            fflush(stdout);
            Elf32_Rel *elfRelocationTable = (Elf32_Rel *)(map_start + sections[i].sh_offset);
            int length = sections[i].sh_size / sections[i].sh_entsize;
            char *secName = section_names + sections[i].sh_name;
            printf("Relocation section '%s' at offset 0x%x contains %d entries: \n", secName, sections[i].sh_offset, length);
            printf("Offset\t\tInfo\t\tType\tSym.Value\tSym.Name\n");
            for (int j = 0; j < length; j++)
            {
                int type = ELF32_R_TYPE(elfRelocationTable[j].r_info);
                int symIndex = ELF32_R_SYM(elfRelocationTable[j].r_info);
                if (debug_mode == 1)
                    fprintf(stderr, "the reloc section name is: %s\n", secName);
                printf("%08x\t", elfRelocationTable[j].r_offset);
                printf("%08x\t", elfRelocationTable[j].r_info);
                // printf("%d\t", elfRelocationTable[j].r_addend);
                printf("%d\t", type);
                printNameAndValue(symIndex);
            }
            printf("\n");
        }
    }
